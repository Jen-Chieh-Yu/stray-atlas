...
### 陷阱四：把「已絕育的待得比較久」讀成絕育讓動物更難送出去

`/analysis` 的分組圖裡，已絕育犬貓的在所天數中位數明顯高於未絕育。照字面讀，會得到一個荒謬的結論。

**實際上因果方向相反**：絕育多半是動物進所之後才安排的，待得越久，越有時間排到手術。這一組量到的是「在所時間造成絕育」，不是「絕育造成在所時間」。

**因此**這一組仍留在圖上，但說明卡直接寫出這件事。刪掉它比較安全，卻會讓讀者自己在別處算出同一個數字，而且沒有這段提醒。

同一頁的下一張圖是相反的示範：全國彙總的毛色差距有一個同樣明顯的替代解釋（深色犬可能集中在原本就滯留較久的收容所），所以那一段**改在同一間收容所之內比較**，並把方向相反的收容所一併留在圖上。控制得掉的就控制，控制不掉的就寫清楚。
