...
- [x] Vue.js 網頁最小可 demo 版
  - [x] **先做縣市 choropleth**（以收容所縣市為準，100% 覆蓋、零推論）
  - [x] 收容所圖釘層（位置為行政區形心，非門牌；頁面上明示），可縮放平移，
        點擊進入該所的動物列表
...
- [x] 在所天數分布頁（`/analysis`）：長條圖＋KDE、對數／線性軸、三段頻寬、
      犬貓 ECDF、分位數表、各縣市排行
  - 頻寬必須公開讓讀者切換，不可只給一條曲線。撐不過平滑的結構不當成發現
  - 可引用的數字一律讀 ECDF，不讀 KDE
- [x] 資料品質評分卡（§9.1 第 5 項）獨立成 `/quality`：四項登錄指標，門檻手選、寫進
      `stats/quality.json`，樣本不足不評級，不給總分
- [x] 分組中位數對照（§9.1 第 1 項，品種犬 vs 混種犬併入八組分類）與
...
```
public/data/
  meta.json               # 快照日期、清理報告、各欄位覆蓋率
  areas.json              # 縣市代碼與名稱對照
  counties.geojson        # 縣市界，由鄉鎮區界溶合而成（RFC 7946 繞向）
  districts.geojson       # 鄉鎮區界（368 個），保留供之後的區級圖層使用
...
原規劃的 `points.geojson`（高信心尋獲點位）**未產出，也不會產出**：達到 high 信心的僅 10 筆（0.1%），畫出來是編碼品質的地圖而不是動物的地圖。相關判斷見 README〈尋獲地：為什麼沒有熱區圖〉。
...
**暫存產物放 `.notes/`，該目錄不進版控。** 設計過程中的原型、模擬畫面、截圖放這裡；桌面用戶端預設寫入的 `Claude outputs/` 同樣排除，避免資料夾重新出現後被 `git add -A` 掃進去。值得保留的結論寫進 `DESIGN.md`，不要以圖片形式 commit。
...
3. **尋獲地覆蓋率與信心分級的說明** — 誠實標示有多少比例無法可靠定位，以及因此**不畫**哪一張圖
